...
export default class Responses {
   ...
   */
  static async comments(comment: CommentDoc[]) {
    const authors = await User.idsToUsernames(comment.map((comment) => comment.author));
    return comment.map((comment, i) => ({ ...comment, author: authors[i] }));
  }
